                    ...
                    <form action="{{route('m_update', $movie)}}" method="post" enctype="multipart/form-data">
                        <div class="input-group mb-3">
                            ...
                            <input type="text" name="maker" class="form-control"
                                value="{{old('maker', $movie->maker)}}">
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text">Plate</span>
                            <input type="text" name="plate" class="form-control"
                                value="{{old('plate' , $movie->plate)}}">
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text">Make year</span>
                            <input type="text" name="make_year" class="form-control"
                                value="{{old('make_year', $movie->make_year)}}">
                        </div>
                        ...
                        <select name="mechanic_id" class="form-select mt-3">
                            ...
                            <option value="{{$mechanic->id}}" @if($mechanic->id == old('mechanic_id', $movie->mechanic_id))
                                selected
                                @endif
